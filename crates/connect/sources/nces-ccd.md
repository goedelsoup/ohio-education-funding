...
Agency files are per-year zips whose column sets change, and `identifier_changes` — the reason
this connector was approved despite overlapping with `dew-foundation` on enrollment — is not
published directly at all. It has to be derived by diffing consecutive years. That derivation
is the work, and it is not started.
