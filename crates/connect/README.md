...
Nine connectors were approved at genesis. The registry now holds **21**, most of them added in
their own [decision record](../../.yidam/decisions/). Four are not: `dew-scholarship-reports`,
`dew-school-improvement`, `ohio-auditor` and `lsc-catalog` appear in the registry and in no
decision record that names them — `lsc-catalog` is the closest to covered, since
[`the-catalog-of-budget-line-items`](../../.yidam/decisions/the-catalog-of-budget-line-items.yml)
scopes the source without naming the key.

The test that guards this list —
`registry::tests::every_connector_approved_in_the_ontology_is_present` — checks the registry
against a hand-written array, not against `.yidam/decisions/`, so it cannot catch the four above.
That is worth saying plainly: it is the same shape of defect as a `[verified]` tag nothing
recomputes.

For eleven phases all nine originals were directories holding a README that described a retrieval
interface nobody had written. That is now a [registry](src/registry/mod.rs) that runs, and each
connector carries a **status that a test checks** rather than a status that a reader has to take
on faith.
