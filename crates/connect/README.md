...
Nine connectors were approved at genesis. The registry now holds **21**, each of them approved in
a [decision record](../../.yidam/decisions/) that names it — and the two tests that guard the list
read those records rather than restating what they say.

`registry::tests::every_connector_in_the_registry_was_approved_in_a_decision_record` derives the
approval list from the `connectors:` field each approving record carries;
`every_connector_approved_in_a_decision_record_is_present` checks the other direction. So a
connector cannot enter the registry without a record, and cannot leave it without its approval
being withdrawn where the argument for keeping it is written down.

**That sentence was false for eleven phases and is a repair rather than a description.** The test
compared the registry against a hand-written array of the same 21 keys, which was extended
alongside each connector — so `dew-scholarship-reports`, `dew-school-improvement`, `ohio-auditor`
and `lsc-catalog` all reached the registry with no record behind them and passed.
[`what-approves-a-connector`](../../.yidam/decisions/what-approves-a-connector.yml) records the
mechanism, and why the records' prose cannot be read for this: a record cites the connectors it
reasons about far more often than it approves one, and `tax-abstract` is named in seven of them
and approved by one. The four have records now, each opening by saying it was written after the
connector was wired.

Before any of that, all nine originals were directories holding a README that described a
retrieval interface nobody had written. That is now a [registry](src/registry/mod.rs) that runs,
and each connector carries a **status that a test checks** rather than a status that a reader has
to take on faith.

<!-- REGEN: edfund-connect connector-registry
...
|---|---|--:|---|
| [`dew-foundation`](sources/dew-foundation.md) | **wired** | 11 | education-agency, revenue-stream, metric, program |
...
| `dew-scholarship-reports` | **wired**, in part | 3 | program, education-agency, school |
...
| [`ohio-laws`](sources/ohio-laws.md) | **wired** | 36 | legislation, parameter, formula-component |
...
| [`census-f33`](sources/census-f33.md) | **wired** | 17 | metric, education-agency |
...
24 connectors, 237 sources between them. 21 are wired and 3 are not; 11 of the wired ones reach only part of what they feed, and say so below.
...
- `dew-scholarship-reports` — still blocked on: wired for everything the department publishes openly about this channel: the current statewide aggregates, the archived participation series FY1997-FY2013, and per-building eligibility for the current designated list. Two holes are left, and they are different in kind. Per-district *participation* is still a file nobody can fetch — the annual report cites two routes for it and both 404, so the breakdown was published, is still referenced by a current departmental document, and has been withdrawn; see `dew-payment-reports` for the deduct-era half of that gap. And the archived series stops at FY2013 while the annual report starts at 2024-25, so FY2014 through FY2023 has no participation series — LSC's greenbooks quote rounded counts for FY2014, FY2018 and FY2019 and nothing else, which bounds that hole rather than filling it. The designated list is current-edition-only: the four prior filenames its pattern implies all 404
...
- `ofcc-projects` — blocked on: the site refuses a self-identifying agent — an identical 5,248-byte 404 to every path including robots.txt, 200 to a browser string — and its project data is rendered by interactive maps rather than served as files. The alternate publishers are surveyed and none allocates: the capital acts appropriate the programme as two lump lines, and the archive holds only the portal shell
...
They are in [`conventions.rs`](../edfund-core/src/conventions.rs) as tests, not comments, because each one
