...
//! phases, when there was no `yidam` binary in this repository or in the vendored prelude — so
//! those were instructions to run something that did not exist, sitting under headings that
//! promised content.
//!
//! That premise expired. Commit `8db052c` installed a `yidam` binary and put `.yidam/bin` first
//! on PATH, and yidam's own regen claims a block by its marker command. Blocks written
//! `<!-- REGEN: yidam <cmd> -->` were then claimed by two generators that disagree about every
//! one of them, and the instruction the READMEs printed destroyed the block it was written on.
//! The marker now names its owner — `edfund-connect <cmd>` — which is the whole ownership
//! record, and it lives in the documents where a re-vendor cannot reach it. See #124 and the
//! note in `mise.toml`.
//!
...
fn generate(command: &str, root: &Path) -> Option<String> {
    Some(match command {
        "edfund-connect corpus-index" => corpus_index(root),
        "edfund-connect catalog-audit" => catalog_audit(root),
        "edfund-connect crates-index" => crates_index(root),
        "edfund-connect skills-index" => markdown_index(root, ".yidam/skills", "Skill"),
        "edfund-connect agents-index" => markdown_index(root, "agents", "Agent"),
        "edfund-connect bundle-status" => bundle_status(root),
        ...
        "edfund-connect index-status" => index_status(root),
        _ => return None,
    })
}
...
mod tests {
    ...
    #[test]
    fn no_block_names_a_generator_this_repository_does_not_own() {
        // #124. Every block here is `edfund-connect`'s. A marker reading `yidam <cmd>` hands the
        // block to yidam's regen, which has generators for seven of these names and produces
        // different content for all of them — so the two gates could not both be green, and the
        // sentence the README printed rewrote six files including web/README.md's feed contract
        // table. Guarded by a test because the inherited task layer is re-vendored wholesale and
        ...
        for document in DOCUMENTS {
            ...
            for marker in text.split("<!-- REGEN: ").skip(1) {
                let command = marker.lines().next().unwrap_or_default().trim();
                assert!(
                    command.starts_with("edfund-connect "),
                    "{document}: block {command:?} names a generator this repository does not own"
                );
            }
            for line in text.lines().filter(|l| l.starts_with("Regenerated by:")) {
                assert_eq!(
                    line, "Regenerated by: `edfund-connect index`",
                    "{document}: tells the reader to run something that would destroy the block"
                );
            }
        }
    }
    ...
    fn a_status_for_a_system_that_does_not_exist_says_so() {
        let text = generate("edfund-connect index-status", &repository_root()).unwrap();
    ...
    fn an_unknown_block_is_an_error_rather_than_a_silent_skip() {
        assert!(generate("edfund-connect invented-command", &repository_root()).is_none());
    }
