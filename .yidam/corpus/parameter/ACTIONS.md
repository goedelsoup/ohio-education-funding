...
- **Real-terms series.** Return a dollar-denominated parameter's series in constant dollars
  via `deflator`. A base cost that is nominally flat is falling, and the nominal series does
  not show it.
...
- `deflator` normalizes dollar-denominated series for cross-era comparison.
