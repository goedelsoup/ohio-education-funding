 ...
 */
import { apply, totals } from "./policy.js";
...
export function toPolicy(shape) {
    let guarantee;
...
export function compare(bundle, checkpoint) {
    const model = bundle.statewide.minimum_state_share;
    const policy = toPolicy(checkpoint.policy);
    const outcomes = bundle.districts.map((d) => apply(d, policy, d.current_year_adm, model));
    const t = totals(outcomes);
    const differences = [];
    const near = (name, ours, theirs) => {
        if (Math.abs(ours - theirs) > TOLERANCE) {
            differences.push(`${name}: page ${ours.toFixed(2)}, feed ${theirs.toFixed(2)}`);
        }
    };
    const same = (name, ours, theirs) => {
        if (ours !== theirs)
            differences.push(`${name}: page ${ours}, feed ${theirs}`);
    };
