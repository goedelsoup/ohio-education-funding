...
import { escapeHtml, money, ordinal, pct, percentileOf } from "./format.js";
function strip(label, value, sorted) {
    if (value == null)
        return `<p class="note">${escapeHtml(label)}: not reported.</p>`;
...
/** Render one district. */
export function renderDistrict(bundle, d) {
    ...
    if (d.at_millage_floor)
        flags.push("At the 20-mill floor");
    if (d.at_minimum_state_share)
        flags.push("At the minimum state share");
    ...
    <div class="tiles">
      ...
      <div class="tile"><div class="k">State aid / pupil</div>
        ...
        <div class="n">${d.on_guarantee
        ? money(guaranteePP) + " of it from the guarantee"
        : "all from the formula"}</div></div>
    ...
    <div class="card">
      ...
      <div class="scroll"><table><tbody>
        ...
        <tr><th>Effective Class 1 millage</th><td>${d.effective_class1_millage == null ? "—" : d.effective_class1_millage.toFixed(2)}</td></tr>
