 ...
 * `intervention/academic-distress-commission` became the fifty-fourth. 425 at the
 ...
 *
 * Recomputed at **963/78** when #408 opened the profile report's personnel block and found one
 * count. Three bindings, no new carrier, on `formula-component/fsfp-base-cost-calculation` — the
 * node that carries the staffing floors — for the one floor a published count can be set
 * against: how many districts employ fewer administrators than the section funds, and the median
 * employed-to-funded multiple either side of the 1,500 ADM threshold. The finding the bindings
 * cannot hold is the negative one, that six of the seven floors meet no column at all; the
 * dispersion crate asserts that on the extract's header instead. Measured with this test's own
 * expressions: 960 stated, 960 actual, 963 after.
 *
 ...
 * pinned — the endpoint rule reaching backwards into prose that was already there.
  *
 * Recounted at **1063/78** when the district fan was given the level its own band is centred on.
 * One binding is that work — the mean district's bias at six years across the closure, the horizon
 * the fan ends on, which was the only one of the four six-year figures the manifest did not carry
 * — and **sixteen had arrived without the floor moving**, against 1046 declared. So the drift the
 * sections above keep describing has happened a fourth time, and the instruction they keep giving
 * is still the one that was not followed: recompute, never increment.
 *
 * No new carrier. The binding lands on `scenario/guarantee-phase-out`, which already stated the
